...
/**
 * Deletes every baseline/session/sample/rollup for a profile — the "delete everything" half of
 * PLAN.md §7's data-control promise. Deliberately does not touch `settings`/consent: consent is a
...
export async function deleteAllData(db: HaloPulseDb, profileId: string): Promise<void> {
  await db.transaction("rw", db.baselines, db.sessions, db.samples, db.rollups, async () => {
    ...
    await db.rollups.where("profileId").equals(profileId).delete();
  });
}
